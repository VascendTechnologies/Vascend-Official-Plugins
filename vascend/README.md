...
- `plan.js` / `mark.js` / `validate.js` — CLI: crea il piano (accetta `@dep:T01,T02` per le dipendenze), accende un bit (`--dry` anteprima, `--note "<t>"` annota, `--check "<cmd>"` gate di verifica con timeout, `--force` bypassa dipendenze e gate After), emette il verdetto (`--deep` figli ricorsivi + coerenza roll-up, `--kingdom [--json]` tutti i castelli). I cwd vengono canonicalizzati (realpath: case e short-name 8.3 di Windows non creano più goal fantasma).
- `castle.js` — **castelli multipli**: `new <slug> … [--after <slug>]` (anti-ciclo sul DAG After), `list`, `map`, `next` (prossima stanza al buio del regno), `drop`, `kanban [--write]` (board fatto/in corso/al buio/fallito → `VASCEND_KANBAN.md`), `mermaid` (grafo del regno), `stats [--json]` (durate per stanza dalla cronologia della Trace: piano più lento, stanze candidate a sub o `@compact`).
- `unmark.js` — annulla una marcatura sbagliata: appende una riga `UNDO` firmata che spegne il bit (append-only, tracciato).
- `resume.js` — riprende un REGNO aperto di un'altra sessione: `--list`, anteprima, `--attach` (riporta master + castelli + sotto-piani + appunti sulla sessione corrente).
- `prune.js` — retention del goalDir: rimuove i regni CHIUSI (conformi o skeleton) più vecchi di N giorni (`--days`, default 14, env `DANILOV_GOAL_RETENTION_DAYS`, `0` = off; `--dry` anteprima). Mai i regni aperti né la sessione corrente. Invocato in automatico (best-effort) a ogni SessionStart.
